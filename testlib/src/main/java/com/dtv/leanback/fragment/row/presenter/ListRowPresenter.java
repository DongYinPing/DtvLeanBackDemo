 ...
 * the License.
 */
package com.dtv.leanback.fragment.row.presenter;
...
import com.dtv.leanback.R;
import com.dtv.leanback.fragment.row.adapter.ItemBridgeAdapter;
import com.dtv.leanback.fragment.row.focus.FocusHighlight;
import com.dtv.leanback.fragment.row.focus.FocusHighlightHelper;
import com.dtv.leanback.fragment.row.model.ListRow;
import com.dtv.leanback.fragment.row.shadow.ItemBridgeAdapterShadowOverlayWrapper;
import com.dtv.leanback.fragment.row.shadow.ShadowOverlayHelper;
import com.dtv.leanback.fragment.row.view.ListRowView;
import com.dtv.leanback.system.Settings;
import com.dtv.leanback.widget.gridview.BaseGridView;
import com.dtv.leanback.widget.gridview.HorizontalGridView;
import com.dtv.leanback.widget.gridview.OnChildSelectedListener;
...
public class ListRowPresenter extends RowPresenter {
    ...
    public static class SelectItemViewHolderTask extends ViewHolderTask {
        ...
        public void run(Presenter.ViewHolder holder) {
            if (holder instanceof ViewHolder) {
                ...
                com.dtv.leanback.widget.gridview.ViewHolderTask task = null;
                if (mItemTask != null) {
                    task = new com.dtv.leanback.widget.gridview.ViewHolderTask() {
    ...
    private static void initStatics(Context context) {
        if (sSelectedRowTopPadding == 0) {
            ...
            sExpandedSelectedRowTopPadding = context.getResources().getDimensionPixelSize(
                    R.dimen.lb_browse_expanded_selected_row_top_padding);//16x1.33 = 21
    ...
    //TODO XXX 不处理好会卡 sExpandedSelectedRowTopPadding 默认也8
    private void setVerticalPadding(ViewHolder vh) {
        ...
        if (vh.isExpanded()) {
           int headerSpaceUnderBaseline = getSpaceUnderBaseline(vh);
            if (DEBUG) Log.v(TAG, "sExpandedSelectedRowTopPadding " + sExpandedSelectedRowTopPadding);
            if (DEBUG) Log.v(TAG, "vh.mPaddingTop " + vh.mPaddingTop);
            ...
            if (DEBUG) Log.v(TAG, "isSelected " + 2);
            paddingTop = sSelectedRowTopPadding - vh.mPaddingBottom;
            paddingBottom = sSelectedRowTopPadding;
        } else {
            if (DEBUG) Log.v(TAG, "!isExpanded !isSelected " + 3);
            paddingTop = 0;
            paddingBottom = vh.mPaddingBottom;
        }
        if (DEBUG) Log.v(TAG, "setPadding " + vh.mPaddingLeft +"/"+paddingTop
                +"/"+vh.mPaddingRight
                +"/"+  paddingBottom);
    ...
    protected void onRowViewSelected(RowPresenter.ViewHolder holder, boolean selected) {
        Log.d(" onRowViewSelected  ","ListRow: "+selected);
    ...
    private void updateFooterViewSwitcher(ViewHolder vh) {
        if (vh.mExpanded && vh.mSelected) {
            //TODO RM BY DONG mHoverCardPresenterSelector
            if (mHoverCardPresenterSelector != null) {
            ...
            //TODO RM BY DONG mHoverCardViewSwitcher
            /*if (mHoverCardPresenterSelector != null && vh.mHoverCardViewSwitcher!= null ) {
    ...
    private void setupFadingEffect(ListRowView rowView) {
        ...
        if (mBrowseRowsFadingEdgeLength < 0) {
            mBrowseRowsFadingEdgeLength = 0;
            //TODO RM BY DONG LeanbackTheme_browseRowsFadingEdgeLength
